...
## b2f830b 
一通り三角形を実装できた（はず）。
まだ1枚しか出してないので、もうちょっといっぱい表示したい。

## 1373cc1
三角形の描画のテストとして、必ず出てくるだろう例のティーポットを描画した。

![回るティーポット](./image/8.gif)

obj形式の頂点番号は1始まりなのに気づかず変なモデルが出たりしたが、それを修正したら正しく表示された。

ところで、デバッグする過程でBVH周りを疑い、それなしで（線形探索で）描画をしてみたら90秒もかかった。
BVHありだと3秒程度なので、プリミティブの数が増えると恩恵が大きくなるのは正しそうだ。

とはいえ、1フレームに1秒以上かかるのは普通に困るので、なんらかの最適化を試みたくはある。

## 6e2e656
本当に愚かでお恥ずかしい話なのだが、BVHの構築法を見たらX軸のみで分割するままになっていた。

ということで最長の辺を使うようにした所、各フレームの描画時間が一気に短縮され、
1つ前のコミットで20秒かかったレンダリングは5秒弱になった。
...
ひとまず画像の読み込みは後回しにしてuv座標ができてるかの確認。

下のgifでは、uv座標を元にRGB値(u, 128, v)となるテクスチャを表示させている。


![虹色の玉と板](./image/9.gif)

## 31f0779
コンパイル時間が長い気がしてならない。ということで、プリコンパイルヘッダとかを使おうと思う。

コンパイル時間は6秒くらいから3秒くらいになった。
また、その関係でMakefileでビルドするようにした。

## HEAD
テクスチャをランバート反射の色として表示できた。地球儀くるくる。
現状、小数点以下の切り捨てでテクスチャを参照しているので、気が向いたら補間をつける。

Ray Tracing: The Next Weekではテクスチャの画像の保持方法が、まず0〜1のfloatで読み込み、uint8で保持し、読みだしたらまた0〜1のfloatに戻す…という流れになっている。
意図がいまいちよくわからなかったのでfloatのまま保持している。

uint8にする利点はキャッシュラインに収まる確率が高くなるとかそのあたりかと予想しておく。
しかし、適当に試した中では速度は大して変わらなかったので、もう少し規模が大きくなってから試す。
